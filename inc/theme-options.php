...
function theme_options_init(){
	register_setting( 'blackpirates_options',
                'blackpirates_theme_options', 
...
function theme_options_add_page() {
	add_theme_page( __( 'Theme Options', 'blackpirates' ),
                        __( 'Theme Options', 'blackpirates' ), 
                       'edit_theme_options', 
                        'theme_options', 
...
function theme_options_do_page($tab = '') {
	...
	<div class="wrap">            
            ...
            <?php echo "<h2>" . wp_get_theme().': ' . __( 'Konfiguration ändern', 'blackpirates' ) . "</h2>"; ?>

            <?php if ( false !== $_REQUEST['settings-updated'] ) : ?>
            <div class="updated fade"><p><strong><?php _e( 'Neue Konfiguration gespeichert.', 'blackpirates' ); ?></strong></p></div>
            <?php endif; 
        ...
        if (!isset($setoptions['blackpirates_theme_options'][$tab])) {
           // echo "Invalid Tab-Option or undefined Option-Field $tab";            
	    $tab = array_keys($setoptions['blackpirates_theme_options'])[0];
        }        

        
        echo "<h3 class=\"nav-tab-wrapper\">\n";
        foreach($setoptions['blackpirates_theme_options'] as $i => $value) {                
        ...
        <form method="post" action="options.php">
            <?php settings_fields( 'blackpirates_options' ); ?>
            <input type="hidden" id="blackpirates_theme_options[tab]" name="blackpirates_theme_options[tab]"  value="<?php echo $tab; ?>">                       
          
          
        <div id="einstellungen">                                       
	<table>	
                <?php
                    if (isset($setoptions['blackpirates_theme_options'][$tab]['fields'])) {
                        foreach($setoptions['blackpirates_theme_options'][$tab]['fields'] as $i => $value) {   
                            $name = $i;
			    $mark_option =0;
				...
				echo "\t\t\t<table class=\"suboptions\">\n";      
			       ...
			       echo "\">\n\t\t<th scope=\"row\">";
                               ...
                               echo "</th>\n\t\t<td>";
                                ...
                                if ($type =='bool') {
                                    echo "\t\t\t";
                                    echo "<input id=\"$name\" name=\"blackpirates_theme_options[$name]\" 
				    ...
				    echo ">\n";
                                    ...
                                    echo "\t\t\t";
                                    echo "<input class=\"regular-text\" id=\"blackpirates_theme_options[$name]\" 
                                            type=\"text\" name=\"blackpirates_theme_options[$name]\" 
					    value=\"";
				    if (isset($options[$name])) echo esc_attr( $options[$name] );				
				    
				    echo "\"><br>\n";
                                    echo "\t\t\t";
                                    echo "<label for=\"blackpirates_theme_options[$name]\">$label</label>\n";
				} elseif (($type=='html') ||($type=='url')) {
                                    echo "\t\t\t";
                                    echo "<input class=\"large-text\" id=\"blackpirates_theme_options[$name]\" 
                                            type=\"text\" name=\"blackpirates_theme_options[$name]\" 
					    size=\"120\" value=\"";
				    if (isset($options[$name])) echo esc_attr( $options[$name] );				
				    ...
				    if (isset($options[$name])) {
					    echo "<img class=\"imgurl\" src=\"".esc_attr( $options[$name] )."\" alt=\"\">\n";
                                    ...
                                    echo "\t\t\t";                                                                                                            
                                    echo "<textarea class=\"large-text\" id=\"blackpirates_theme_options[$name]\" 
                                            cols=\"30\" rows=\"10\"  name=\"blackpirates_theme_options[$name]\">";
				    if (isset($options[$name])) echo esc_attr( $options[$name] );
				    echo "</textarea><br>\n";
                                    echo "\t\t\t";
                                    echo "<label for=\"blackpirates_theme_options[$name]\">$label</label>\n";    
				    ...
				    <input type="hidden" name="blackpirates_theme_options[<?php echo $name; ?>]" id="file_<?php echo $name; ?>" 
					     value="<?php if (isset($options[$name])) echo sanitize_key( $options[$name]); ?>" />
				    
				    <input type="text" name="blackpirates_theme_options[<?php echo $name; ?>_url]" id="file_<?php echo $name; ?>_url" 
					   value="<?php if (isset($options[$name])) echo wp_get_attachment_url( esc_attr( $options[$name]) ); ?>" />
				    <input class="button" name="file_button_<?php echo $name; ?>" id="file_button_<?php echo $name; ?>" value="<?php _e('Add file', 'blackpirates'); ?>" />
				    <small><a href="#" class="file_remove_<?php echo $name; ?>"><?php _e( "Remove file", 'blackpirates' );?></a></small>
				    <br><label for="blackpirates_theme_options[<?php echo $name; ?>]"><?php echo $label; ?></label>
				    </div><script>
					...
					$addstyle .= 'max-height: '.$value['maxheight'].'px;';
				     ...
				     if ((isset($options[$name])) && esc_url( $options[$name])) { 
					  ...
					  echo '/>';
					   ...
					   _e('No Image selected', 'blackpirates');
				     }				   
				    ?>		
				    </div>
				    <input type="hidden" name="blackpirates_theme_options[<?php echo $name; ?>_id]" id="image_<?php echo $name; ?>_id" 
					     value="<?php if ( isset( $options[$name."_id"] ) ) echo sanitize_key( $options[$name."_id"] ) ; ?>" />
				    
				    <input type="text" name="blackpirates_theme_options[<?php echo $name; ?>]" id="image_<?php echo $name; ?>" value="<?php if ( isset( $options[$name] ) ) echo esc_attr( $options[$name] ) ; ?>" />
				    <input class="button" name="image_button_<?php echo $name; ?>" id="image_button_<?php echo $name; ?>" value="<?php _e('Add Image', 'blackpirates'); ?>" />
				    <small><a href="#" class="image_remove_<?php echo $name; ?>"><?php _e( "Remove image", 'blackpirates' );?></a></small>
				    <?php if (isset($value['default']) && (filter_var($value['default'], FILTER_VALIDATE_URL))) { ?>
					<small><a href="#" class="image_reset_<?php echo $name; ?>"><?php _e( "Reset to default", 'blackpirates' );?></a></small>
				    <?php } ?>
					...
					jQuery('.image_remove_<?php echo $name; ?>').click(function()   {
						...
						jQuery('.showimg_<?php echo $name; ?>').html('<?php _e('No Image selected', 'blackpirates'); ?>');
						return false;
					});
				    });
				    <?php if (isset($value['default']) && (filter_var($value['default'], FILTER_VALIDATE_URL))) { ?>
				    ...
				    <?php } ?>
                                    ...
                                    echo "\t\t\t";
                                    echo "<input class=\"number\" size=\"5\" id=\"blackpirates_theme_options[$name]\" 
                                            type=\"text\" name=\"blackpirates_theme_options[$name]\" 
				   ...
				   echo "\t\t\t";                                    
					...
					echo "\">\n";
					    ...
					    name="blackpirates_theme_options['.$name.']"';
                                        if ( $src == $options[$name] ) {
				   ...
				   echo "\t\t\t";                      
                                    foreach ( $liste as $option ) {    
                                        ...
                                        <label class="plakattile">
                                            ...
                                            <input type="checkbox" name="blackpirates_theme_options[<?php echo $name?>][]" 
				   ...
				   echo "\t\t\t";                      
					...
					$value = '';
					 ...
					 if (($active==1) && (filter_var($value, FILTER_VALIDATE_URL))) {
					    ...
					    <label for="blackpirates_theme_options[<?php echo $name?>][<?php echo $entry?>][active]" class="<?php echo $entry?>" style="width: 120px; display: inline-block;">
                                            <input type="checkbox" 
						   id="blackpirates_theme_options[<?php echo $name?>][<?php echo $entry?>][active]"
						   name="blackpirates_theme_options[<?php echo $name?>][<?php echo $entry?>][active]" 
                                                   value="1" <?php echo $checked; ?>>                                                                                               
					    <?php echo $liste[$entry]['name'] ?>
					    </label>
					    
                                            <input id="blackpirates_theme_options[<?php echo $name?>][<?php echo $entry?>][content]" 
                                            type="text" name="blackpirates_theme_options[<?php echo $name?>][<?php echo $entry?>][content]" 
					...
					echo "<p>".$label."</p>\n";
                                    ...
                                    echo "\t\t\t";
                                    echo "<select id=\"$name\" name=\"blackpirates_theme_options[$name]\">\n";
					...
					$value = '';                                      
                                         ...
                                         ?>       
					    <label for="blackpirates_theme_options[<?php echo $name?>][<?php echo $entry?>]" >                                                
                                            <input type="checkbox" 
						   id="blackpirates_theme_options[<?php echo $name?>][<?php echo $entry?>]"
						   name="blackpirates_theme_options[<?php echo $name?>][<?php echo $entry?>]" 
					...
					echo "<p>".$label."</p>\n";
                                    ...
                                    echo "\t\t\t";
                                    echo "<select name=\"blackpirates_theme_options[$name]\">\n";
                                    foreach($liste as $i => $value) {   
                                        echo "\t\t\t\t";
                        ...
                        _e( 'Option nicht definiert.', 'blackpirates' );
                    }
                ?>
        ...
        <p class="submit">
                <input type="submit" class="button-primary" value="<?php _e( 'Update', 'blackpirates' ); ?>" />
        </p>
</form>               
</div>
...
function theme_options_validate( $input ) {
    ...
    $options = get_option( 'blackpirates_theme_options' );
    
    $saved = (array) get_option( 'blackpirates_theme_options' );	
        //    $options= $saved;
	...
	delete_option('blackpirates_theme_options');
